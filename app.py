...
att_p = (
    pd.read_csv("data/attending_phy_count.csv", header=0, index_col=0)
    .squeeze("columns")
    .to_dict()
)
att_op = (
    pd.read_csv("data/operating_phy_count.csv", header=0, index_col=0)
    .squeeze("columns")
    .to_dict()
)
att_ot = (
    pd.read_csv("data/other_phy_count.csv", header=0, index_col=0)
    .squeeze("columns")
    .to_dict()
)
prv_csv = (
    pd.read_csv("data/provider_phy_count.csv", header=0, index_col=0)
    .squeeze("columns")
    .to_dict()
)
...
def app():
    ...
    def csv():
        ...
        selected_example = st.selectbox(
            "Download example CSV file:", list(examples.keys())
        )
        if selected_example:
            ...
            st.download_button(
                label="Download",
                data=file_contents,
                file_name=examples[selected_example],
            )
        ...
        if uploaded_file is not None:
            ...
            df["Att_Opr_Oth_Phy_Tot_Claims"] = df.apply(
                lambda row: convert(
                    row["attendingphysician"],
                    row["operatingphysician"],
                    row["otherphysician"],
                ),
                axis=1,
            )
            df["Prv_Tot_Att_Opr_Oth_Phys"] = df.apply(
                lambda row: convert2(row["provider"]), axis=1
            )
            ...
            fraudulent = df.loc[
                df["Potential_Fraud"] == 1, ["claimid", "Potential_Fraud"]
            ]
            ...
            ax.legend(
                title="Legend",
                loc="best",
                labels=["Not Fraud", "Potential Fraud"],
                framealpha=0,
    ...
    def forms():
        with st.form("medi_claim_form"):
            ...
            noofmonths_partacov = st.selectbox(
                "Part A Coverage (in months)", list(range(1, 13))
            )
            noofmonths_partbcov = st.selectbox(
                "Part B Coverage (in months)", list(range(1, 13))
            )
            ...
            chroniccond_heartfailure = (
                1 if st.checkbox("chroniccond_heartfailure") else 0
            )
            chroniccond_kidneydisease = (
                1 if st.checkbox("chroniccond_kidneydisease") else 0
            )
            chroniccond_cancer = 1 if st.checkbox("chroniccond_cancer") else 0
            chroniccond_obstrpulmonary = (
                1 if st.checkbox("chroniccond_obstrpulmonary") else 0
            )
            ...
            chroniccond_ischemicheart = (
                1 if st.checkbox("chroniccond_ischemicheart") else 0
            )
            chroniccond_osteoporasis = (
                1 if st.checkbox("chroniccond_osteoporasis") else 0
            )
            chroniccond_rheumatoidarthritis = (
                1 if st.checkbox("chroniccond_rheumatoidarthritis") else 0
            )
            chroniccond_stroke = 1 if st.checkbox("chroniccond_stroke") else 0
            inscclaimamtreimbursed = st.slider(
            ...
            deductibleamtpaid = st.slider(
                "Deductible Amount Paid", min_value=0, max_value=10000, value=0, step=1
            )

            inpt = 1 if st.checkbox("In Patient") else 0
            los = st.number_input("Length of stay")
            ipannualreimbursementamt = st.slider(
                "In patient Annual Reimbursement Amount",
                min_value=0,
                max_value=10000,
                value=0,
                step=1,
            )
            ipannualdeductibleamt = st.slider(
                "In patient annual Deductible Amount",
                min_value=0,
                max_value=10000,
                value=0,
                step=1,
            )
            opannualreimbursementamt = st.slider(
                "Out patient annual Reimbursement Amount",
                min_value=0,
                max_value=10000,
                value=0,
                step=1,
            )
            opannualdeductibleamt = st.slider(
                "Out patient annual Deductible Amount",
                min_value=0,
                max_value=10000,
                value=0,
                step=1,
